...
app.post('/api/etf/deploy', async (req, res) => {
  try {
    ...
    let createdSheets = {};
    const hasGoogleOAuth = req.session?.google_access_token || configData.google_access_token;
    ...
    if (req.body.template_id === 'pet_clinic_test' || req.body.test_mode) {
      ...
      try {
        ...
        const petWorkflows = workflows.data ? 
          workflows.data.filter(workflow => {
            const workflowTags = workflow.tags || [];
            return Array.isArray(workflow.tags) && workflow.tags.some(tag => {
              if (typeof tag === 'string') {
                return tag.toLowerCase() === 'pet';
              } else if (tag && typeof tag === 'object' && tag.name) {
                return tag.name.toLowerCase() === 'pet';
...
// Create Google OAuth credentials in N8N
...
app.post('/api/etf/deploy', async (req, res) => {
  try {
    ...
    if (hasGoogleOAuth) {
      console.log('🔗 Google OAuth detected, creating required Google Sheets...');
      try {
        createdSheets = await createRequiredGoogleSheets(client_data, req.session?.google_access_token || configData.google_access_token);
...
async function personalizeMultipleWorkflows(templateIds, configData, clientData) {
  ...
  for (const templateId of templateIds) {
    try {
      ...
      console.error(`❌ Failed to create base workflow ${templateId}:`, error.message);
      errors.push({
        originalId: templateId,
        error: error.message,
        success: false
...
async function personalizeMultipleWorkflows(workflowIds, configData, clientData) {
  ...
  for (const workflowId of workflowIds) {
    try {
      ...
      console.error(`❌ Failed to create base workflow ${workflowId}:`, error.message);
      errors.push({
        originalId: workflowId,
        error: error.message,
        success: false
