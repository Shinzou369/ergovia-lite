...
const telegramLoginSessions = new Map();

// Initialize Telegram auth session
app.post('/api/auth/telegram/init', async (req, res) => {
  try {
    const { bot_token, chat_id, bot_username, login_code } = req.body;

    if (!bot_token || !chat_id || !login_code) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    // Store session with expiration
    telegramLoginSessions.set(login_code, {
      bot_token,
      chat_id,
      bot_username,
      created_at: Date.now(),
    ...
    console.log('🔐 Telegram login session initialized:', { chat_id, bot_username, login_code });

    res.json({ success: true, message: 'Login session initialized' });
  } catch (error) {
...
app.get('/api/auth/telegram/verify', async (req, res) => {
  try {
    const { code, chat_id } = req.query;

    if (!code || !chat_id) {
      return res.redirect('/telegram-auth?error=invalid_request');
    }

    const session = telegramLoginSessions.get(code);
    if (!session) {
      return res.redirect('/telegram-auth?error=invalid_code');
    }

    if (Date.now() > session.expires_at) {
      telegramLoginSessions.delete(code);
      return res.redirect('/telegram-auth?error=expired_code');
    }

    if (session.chat_id.toString() !== chat_id.toString()) {
      return res.redirect('/telegram-auth?error=invalid_chat');
    }

    // Get user info from Telegram
    try {
      const chatResponse = await fetch(`https://api.telegram.org/bot${session.bot_token}/getChat?chat_id=${chat_id}`);
      ...
      const telegramUser = chatData.result;
      ...
      const userData = {
        ...
        telegram_bot_token: session.bot_token,
        telegram_bot_username: session.bot_username,
        email: telegramUser.username ? `${telegramUser.username}@telegram.local` : `${telegramUser.id}@telegram.local`,
        authMethod: 'telegram',
        createdAt: new Date().toISOString(),
        isComplete: !!(telegramUser.first_name && telegramUser.last_name)
      };
      ...
      if (existingUser) {
        ...
        savedUser = { ...existingUser, ...userData, googleId: existingUser.googleId };
        saveUser(savedUser);
      } else {
        // Create new user
        savedUser = { ...userData, googleId: userData.telegram_id.toString() };
        saveUser(savedUser);
      }

      // Set session
      req.session.user = savedUser;
      req.session.telegram_authenticated = true;
      
      // Mark session as authenticated
      session.authenticated = true;
      session.user_data = savedUser;

      console.log('✅ Telegram authentication successful:', userData.email);
      ...
      res.redirect('/telegram-auth?success=true');

    } catch (telegramError) {
      console.error('❌ Telegram API error:', telegramError);
      res.redirect('/telegram-auth?error=telegram_api_error');
    }

  } catch (error) {
    console.error('❌ Telegram verify error:', error);
    res.redirect('/telegram-auth?error=server_error');
  }
});
...
// Telegram logout
app.post('/api/auth/telegram/logout', (req, res) => {
  try {
    req.session.destroy((err) => {
      if (err) {
...
// Serve Telegram auth page
app.get('/telegram-auth', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'telegram-auth.html'));
});

// Serve Telegram automation setup page
app.get('/telegram-automation', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'telegram-automation.html'));
});

// Automatic bot creation and management
app.post('/api/telegram/create-automation-bot', async (req, res) => {
  try {
    const { business_name, business_type = 'general', features = [] } = req.body;

    if (!req.session.user || !req.session.user.telegram_bot_token) {
      return res.status(401).json({ error: 'Telegram authentication required' });
    }

    const botToken = req.session.user.telegram_bot_token;
    const chatId = req.session.user.telegram_chat_id;

    // Set up bot commands based on business type
    const commands = generateBotCommands(business_type, features);
    ...
    // Send welcome message with business info
    const welcomeMessage = generateWelcomeMessage(business_name, business_type, commands);
    ...
    await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
      ...
      body: JSON.stringify({
        ...
        parse_mode: 'Markdown',
        reply_markup: {
          keyboard: generateKeyboard(business_type),
    ...
    // Create N8N credential for this bot
    const credentialResult = await createTelegramN8NCredential(
    ...
    res.json({
      success: true,
      message: 'Automation bot configured successfully!',
      bot_username: req.session.user.telegram_bot_username,
      commands: commands,
      credential_id: credentialResult.credentialId,
      features: [
        'Automated business responses',
        'Customer inquiry handling',
        'Appointment scheduling (if configured)',
    ...
    console.error('❌ Bot automation setup error:', error);
    res.status(500).json({ error: 'Failed to set up automation bot' });
  }
});

function generateBotCommands(businessType, features) {
  const baseCommands = [
    { command: 'start', description: 'Start the bot and see menu' },
    { command: 'help', description: 'Get help and available commands' },
    { command: 'hours', description: 'Check business hours' },
  ...
  const businessCommands = {
    'dental': [
      ...
      { command: 'emergency', description: 'Emergency contact info' },
      { command: 'reminders', description: 'Set appointment reminders' }
    ],
    'gym': [
      { command: 'classes', description: 'View class schedule' },
      { command: 'membership', description: 'Membership information' },
      { command: 'book', description: 'Book a class or session' }
    ],
    'restaurant': [
      { command: 'menu', description: 'View menu' },
      { command: 'order', description: 'Place an order' },
      { command: 'reservation', description: 'Make a reservation' }
    ]
  };

  return [...baseCommands, ...(businessCommands[businessType] || [])];
}

function generateWelcomeMessage(businessName, businessType, commands) {
  const commandList = commands.map(cmd => `/${cmd.command} - ${cmd.description}`).join('\n');
  
  return `🤖 **Welcome to ${businessName} Automation Bot!**

Your business automation is now active! This bot can help with:
...
Type /start to see the main menu or use any command above.

*Your bot is powered by ERGOVIA-AI automation technology.*`;
}

function generateKeyboard(businessType) {
  const baseButtons = [
    ['📞 Contact', '⏰ Hours'],
    ['🔧 Services', '❓ Help']
  ];
  ...
  return businessButtons[businessType] || baseButtons;
}

async function createTelegramN8NCredential(userEmail, botToken, chatId, businessName) {
  try {
    if (!N8N_BASE_URL || !N8N_API_KEY) {
    ...
    const credentialData = {
      name: `Telegram Bot - ${businessName} (${userEmail})`,
      type: 'telegramApi',
      data: {
    ...
    if (response.ok && result.id) {
      console.log(`✅ Created Telegram N8N credential: ${result.id}`);
    ...
    console.error('❌ Error creating Telegram N8N credential:', error);
